...
# .       HasMastered: Don't adapt if student hasn't mastered material.
...
class FSM:
    ...
    def next_speaker_selector(self, last_speaker, groupchat):
        ...
        if self.current_state == "AwaitingTopic":
            ...
            self.current_state = "HasMastery"
            return self.agents["learner_model"]
        
        elif self.current_state == "HasMastery":
            # TODO: Either generate prompts where the agent decides if 
            # .   mastery has been achieved OR
            #  Create a counter and only branch to the AdaptingLevel on Mastery
            # Otherwise go back to awaitingproblem
            self.current_state = "AdaptingLevel"
            return self.agents["mastery"]
