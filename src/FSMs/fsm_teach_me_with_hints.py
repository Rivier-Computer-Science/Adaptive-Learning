...
from src.Agents import agents_hint
...
class TeachMeFSM:
    def __init__(self, agents, max_code_execution_attempts=3):
       ...
       # Transition from VERIFYING_CODE to UPDATING_MODEL if code executes and is correct
        self.machine.add_transition(
            ...
            conditions='student_is_correct_or_code_is_correct_or_too_many_execution_attempts',
            after='set_learner_model'
        )

       # Transition from VERIFYING_CODE to GIVING_HINT
        # Student answered incorrectly
       # Transition from VERIFYING_CODE to UPDATING_MODEL if code executes and is correct
        self.machine.add_transition(
            trigger='advance',
            source=FSMStates.VERIFYING_ANSWER.value,
            dest=FSMStates.GIVING_HINT.value,
            conditions='wrong_student_answer',
            after='give_hint'
        )

        # Keep giving hints until the correct answer is received
        self.machine.add_transition(
            trigger='advance',
            source=FSMStates.GIVING_HINT.value,
            dest=FSMStates.AWAITING_ANSWER.value,
            unless='student_answer_is_correct'
        )

        # Once the correct answer is received, move forward in the FSM
        self.machine.add_transition(
            trigger='advance',
            source=FSMStates.GIVING_HINT.value,
            dest=FSMStates.UPDATING_MODEL.value,
            conditions='student_answer_is_correct',
            after='set_learner_model'
        )
    ...
    def code_is_correct(self):
        ...
        if not all_messages:  # Ensure messages exist
            logging.info("No messages found in groupchat.")
            return False

        # Get the latest message from the groupchat manager
        last_message = all_messages[-1]
        logging.info(f"Evaluating message from sender '{last_message.get('name', 'Unknown')}': {last_message.get('content', '')}")

        # Check if the last message confirms successful execution
        if "code executed successfully" in last_message.get('content', '').lower():
            logging.info("Code run succeeded with exit code 0.")
            return True
    ...
    def wrong_student_answer(self):
        # Logic to check if the student's answer is incorrect
        return not self.student_answer_is_correct()

    def student_answer_is_correct(self):
        # Implement the logic to determine if the answer is correct
        # Example: Compare student's last response with the expected correct answer
        last_message = self.groupchat_manager.groupchat.get_messages()[-1]
        return "Correct!" in last_message['content']  


    def student_is_correct_or_code_is_correct_or_too_many_execution_attempts(self):
        """
        Determines if the student is correct, the code is correct, or too many attempts have been made.
        If the student is wrong, they receive a hint instead of the correct answer.
        """
        if self.too_many_code_execution_attempts():
            self.reset_attempts()
            return True  # Move forward if too many failed attempts

        if self.code_is_correct():
            return True  # Move forward if code execution is correct

        if self.wrong_student_answer():
            self.give_hint()  # Provide a hint instead of progressing
            return False  
    ...
    def give_hint(self):
        """
        Provides a hint to the student when their answer is incorrect.
        """
        try:
            hint_agent = self.agents.get(AgentKeys.HINT_PROVIDER.value)  # Use `.get()` to avoid KeyError
            if not hint_agent:
                logging.error("Hint agent not found.")
                return

            hint_message = hint_agent.get_hint()  # Fetches a hint dynamically
            logging.info(f"Hint provided: {hint_message}")
            self.groupchat_manager.groupchat.send_message(hint_message)
        except Exception as e:
            logging.error(f"Error while providing hint: {e}")
