...
class TeachMeFSM:
    def __init__(self, agents, max_code_execution_attempts=3):
       ...
       # Transition from VERIFYING_CODE to UPDATING_MODEL if code executes and is correct
        self.machine.add_transition(
            ...
            conditions='student_correct_or_code_is_correct_or_too_many_execution_attempts',
    ...
    """
    Check if the student's answer is incorrect by analyzing the groupchat_manager messages.
    """
    if not hasattr(self, 'groupchat_manager') or not self.groupchat_manager:
        logging.error("Groupchat manager not registered or is None.")
        return False
    ...
    # Check the latest message to determine if the student's answer is incorrect
    if all_messages:
        last_message = all_messages[-1]
        logging.info(f"Evaluating student response: {last_message['content']}")
        if "incorrect answer" in last_message['content'].lower():
            logging.info("Student's answer was incorrect.")
            return True

    logging.info("Student's answer was not explicitly marked as incorrect.")
    return False


    def student_is_correct_or_code_is_correct_or_too_many_execution_attempts(self):
    """
    Check if the student is correct, the code execution is correct, or there have been too many execution attempts.
    """
    ...
    if hasattr(self, 'student_is_correct') and callable(self.student_is_correct):
        return self.student_is_correct()
