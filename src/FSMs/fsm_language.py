...
import src.Agents.language_agents as agents  # Updated import
...
class TeachMeFSM:
    def __init__(self, agents_dict, AgentKeys=agents.AgentKeys):
        logging.debug("Initializing TeachMeFSM")
        self.agents = agents_dict  # Renamed to agents_dict for clarity
    ...
    def set_teacher(self):
        try:
            ...
            logging.debug(f"set_teacher(): Next agent set to 'teacher' ({self.next_agent.name})")
    ...
    def next_speaker_selector(self, last_speaker, groupchat):
        ...
        try:
            self.advance()
            logging.info(f"next_speaker_selector(): advance FINISHED. last_speaker= {self.last_speaker.name if self.last_speaker else 'None'}. next_speaker= {self.next_agent.name}")
